...
- **FR-001**: A coherent state root for the synced state MUST be present locally at heal start (the trie is built during download). If it is absent, the node MUST NOT silently skip healing; it MUST fall back to the existing on-demand path as an explicit last resort and surface that it did so. *(US1, US2)*
...
- **FR-003**: The heal MUST discover missing nodes by **top-down delta traversal** (fetch a node → decode → request only its hash-referenced children that are absent locally). It MUST NOT perform an O(total) walk of the entire local trie, and MUST NOT perform a separate second verification walk. Already-present, correct subtrees MUST be pruned (not descended or re-requested). *(US2)*
...
- **SC-003**: The heal performs no O(total) walk of the local trie and no second verification walk; total nodes requested over the heal is on the order of the missing delta, not the full trie (orders of magnitude smaller than total node count).
