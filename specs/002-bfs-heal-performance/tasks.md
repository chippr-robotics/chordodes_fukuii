...
- [ ] T017 [P] [US2] Per-phase timing + inflation counter test on a synthetic walk: three phase-nanos > 0 (once per chunk); `childRefsSeen ≥ distinctEnqueued`; emitted ratio == quotient, in `FrontierRebuildSpec.scala` — DEFERRED: depends on the US3 foundational walk-driving fixture (T003/T004), which does not exist yet (`FrontierRebuildSpec` currently only tests `boundedVisitedSet`). Do not build that fixture under US2.
- [ ] T018 [P] [US2] Observation-only regression: counters/timing do NOT change the frontier/enqueued-children set for a fixed trie (FR-023/SC-006), in `FrontierRebuildSpec.scala` — DEFERRED for the same reason as T017 (no `rebuildFrontierBFS`-driving harness yet). The instrumentation in T025 is observation-only by construction: counters increment beside the existing logic and the `markIfNew`/enqueue arms are unchanged.
