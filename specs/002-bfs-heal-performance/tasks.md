...
- [ ] T003 Add a shared synthetic-trie test fixture (a branch/extension node referenced by two parents, with an injectable missing grandchild) usable by `rebuildFrontierBFS` tests, in `src/test/scala/com/chipprbots/ethereum/blockchain/sync/snap/actors/HealingTrieFixtures.scala`
...
- [ ] T009 [P] [US1] FR-025 regression: a missing node behind a SHARED branch/extension ancestor is still discovered after the visited set reports that ancestor seen (uses T003 fixture), in `src/test/scala/com/chipprbots/ethereum/blockchain/sync/snap/actors/FrontierRebuildSpec.scala`
...
- [ ] T015 [P] [US2] `GcPressureSampler` unit test with injected clock + bean-sum supplier, incl. zero-wall-time guard, in `src/test/scala/com/chipprbots/ethereum/blockchain/sync/snap/actors/GcPressureSamplerSpec.scala`
- [ ] T016 [P] [US2] `RocksDbDataSource` statistics test: N hits + M misses move `cacheStats` tickers; `cacheStats == None` when flag off; `close()` releases the handle, in `src/test/scala/com/chipprbots/ethereum/db/dataSource/RocksDbDataSourceSpec.scala`
- [ ] T017 [P] [US2] Per-phase timing + inflation counter test on a synthetic walk: three phase-nanos > 0 (once per chunk); `childRefsSeen ≥ distinctEnqueued`; emitted ratio == quotient, in `FrontierRebuildSpec.scala`
- [ ] T018 [P] [US2] Observation-only regression: counters/timing do NOT change the frontier/enqueued-children set for a fixed trie (FR-023/SC-006), in `FrontierRebuildSpec.scala`
- [ ] T019 [P] [US2] `SNAPSyncMetrics` setter round-trip via `SimpleMeterRegistry` for the new gauges, in `src/test/scala/com/chipprbots/ethereum/blockchain/sync/snap/SNAPSyncMetricsSpec.scala`

### Implementation for User Story 2

- [ ] T020 [P] [US2] New `GcPressureSampler` (baseline `getCollectionTime` sum + wall clock; `sample(): (deltaPauseMs, fraction)`; injectable clock/bean supplier), in `src/main/scala/com/chipprbots/ethereum/blockchain/sync/snap/actors/GcPressureSampler.scala`
- [ ] T021 [P] [US2] Add `enableStatistics: Boolean = false` to the `RocksDbConfig` trait; read `enable-statistics` (hasPath-guarded) in `InstanceConfig` `object RocksDb`, in `RocksDbDataSource.scala` + `src/main/scala/com/chipprbots/ethereum/utils/InstanceConfig.scala`
- [ ] T022 [US2] In `RocksDbDataSource.createDB`, attach a `Statistics` (`StatsLevel.EXCEPT_DETAILED_TIMERS`) when enabled, retain as an `Option` field, add `cacheStats: Option[(hit,miss,idxHit,idxMiss)]` + `estimate-table-readers-mem`, close in `close()`, in `RocksDbDataSource.scala` (depends on T021)
- [ ] T023 [US2] Add `enable-statistics = false` under `db.rocksdb` with a ~1-2% overhead comment, in `src/main/resources/conf/base/db.conf`
- [ ] T024 [P] [US2] Add SNAPSyncMetrics gauges + setters: `setHealingPhaseQueueReadMs/TrieReadMs/QueueWriteMs`, `setHealingGcPauseMs`, `setHealingGcFraction`, `setHealingInflationRatio`, and poll-closure cache gauges, in `src/main/scala/com/chipprbots/ethereum/blockchain/sync/snap/SNAPSyncMetrics.scala`
- [ ] T025 [US2] In `rebuildFrontierBFS`/`processSubRange`: add per-walk `queueReadNanos/trieReadNanos/queueWriteNanos` + per-level `childRefsSeen/distinctEnqueued`; time the three ops per chunk; increment `childRefsSeen` before each `markIfNew` and `distinctEnqueued` in its true branch; emit all five to T024 setters + the GC sampler at the level boundary and append to the `[HEAL-BFS] Level complete` log; reset per-level counters, in `TrieNodeHealingCoordinator.scala` (depends on T020, T024; same file as US1 → after T010–T013)
...
- [ ] T026 [P] [US3] Parallelism-formula unit test: `min(cfg, max(min, nproc − reserved))` across tuples (4-core ⇒ 2; 16-core ⇒ cfg; `cfg=1` ⇒ 1), in `src/test/scala/com/chipprbots/ethereum/blockchain/sync/snap/actors/TrieNodeHealingCoordinatorSpec.scala`
- [ ] T027 [P] [US3] Serial-equivalence: with `cfg=1` the emitted frontier + enqueue order is byte-identical to the pre-change serial path on a fixed trie (FR-012), in `FrontierRebuildSpec.scala`
- [ ] T028 [P] [US3] Pipeline = serial equivalence: pipelined `processSubRange` returns the same frontier set and enqueued-children set as the serial loop (stub mptStorage records call order), in `FrontierRebuildSpec.scala`
- [ ] T029 [P] [US3] Starvation/deadlock guard: a level spawning N == reader-pool-size sub-ranges completes within a bounded virtual-time budget (no Await deadlock), in `TrieNodeHealingCoordinatorSpec.scala`
- [ ] T030 [P] [US3] Bounded-heap + backpressure: peak buffered chunks ≤ handoff-queue capacity and `awaitFrontierDrain` (#1338) still invoked per batch, in `FrontierRebuildSpec.scala`
- [ ] T031 [P] [US3] Non-overlap invariant property test: after each level, `max read key < min written child key` under concurrent enqueue; shared-ancestor missing node still discovered concurrently (FR-025), in `FrontierRebuildSpec.scala`

### Implementation for User Story 3

- [ ] T032 [US3] Add a dedicated `healing-reader-dispatcher` (fixed pool sized to the parallelism ceiling, default 4–6), in `src/main/resources/conf/base/pekko.conf`
- [ ] T033 [US3] Run the sub-range Futures on the new reader EC (or wrap the parent `Await.result` in `scala.concurrent.blocking { }`) to remove the Await-on-same-pool starvation hazard, in `TrieNodeHealingCoordinator.scala` (do FIRST — blocking prerequisite for raising parallelism)
- [ ] T034 [US3] Replace `effectiveParallelism` with `min(cfg, max(healingMinParallelism, nproc − healingReservedCores))`; never exceed `cfg`/`nproc`; `cfg=1` ⇒ serial, in `TrieNodeHealingCoordinator.scala` (depends on T033)
...
- [ ] T036 [US3] Add `healing-min-parallelism = 2` and `healing-reserved-cores = 2` with the effective-parallelism + pipeline-depth doc note (4-core ref host stays at 2), in `src/main/resources/conf/base/sync.conf`
