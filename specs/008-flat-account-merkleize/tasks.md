...
- [X] T013 [US3] Add a `finalizing` freeze latch to `AccountRangeCoordinator`: while latched, IGNORE `PivotRefreshed` (no `rootHash` re-tag, no re-enqueue). Follow the existing finalization-ignore precedent (`AccountRangeCoordinator.scala:929-930`). (C2) DONE: `private[actors] var finalizeFreezeLatch` + guarded `case PivotRefreshed if finalizeFreezeLatch =>` no-op; `BeginFinalizing` sets it + re-arms completed ranges from `rangeStart`; `EndFinalizing` clears it. Self-review verified the latched `PivotRefreshed` is a true no-op (no drain, no re-tag).
- [X] T014 [US3] Add the symmetric `finalizing` freeze latch to `StorageRangeCoordinator`: while latched, IGNORE `StoragePivotRefreshed` (`StorageRangeCoordinator.scala:~867`). (C2) DONE: symmetric `finalizeFreezeLatch` + guarded `case StoragePivotRefreshed if finalizeFreezeLatch =>`; `BeginStorageFinalizing`/`EndStorageFinalizing`. By-design it only latches+adopts the root (no completed-range re-arm) — slot coherence is import-time, not gate-time (research Decision 1); verified consistent.
- [X] T015 [US3] Implement the finalize freeze+re-fetch in `SNAPSyncController.scala`: pick a servable `R_final` (the committed pivot, servability proven by the per-request proof-verify), send `BeginFinalizing(R_final)`/`BeginStorageFinalizing(R_final)` to both coordinators, re-fetch all account ranges via `GetAccountRange(R_final)` into `FlatAccountStorage` (overwrite stale leaves). Then run the batch-2 gate. DONE: `beginFinalizeRefetch` spawns a fresh full-keyspace coordinator targeted at R_final writing the SAME flat store. **FIX (self-review):** the committed code had `runFinalizeGateAndComplete`/`failFinalizeRefetch` with ZERO callers — the coordinator's `FinalizingRefetchComplete` signal landed in dead letters and the gate never ran (the re-fetch hung in `FinalizeRefetch` forever). Added the missing `case FinalizingRefetchComplete(rFinal)` handler (stale-attempt guarded) → `runFinalizeGateAndComplete`. Also fixed a `MatchError`-on-`FinalizeRefetch` exhaustivity gap in `calculateStateProgress`.
- [X] T016 [US3] Fail-closed path: if peers stop serving `R_final` before the re-fetch completes, do NOT finalize; abort, retry on a fresher `R_final` (bounded), else the existing handoff (FR-004). DONE + **FIX (self-review):** `failFinalizeRefetch` was also never called — during `FinalizeRefetch` the coordinator's `PivotStateUnservable` escalation fell into the controller's `else`/ignore branch. Added `case PivotStateUnservable if currentPhase == FinalizeRefetch => failFinalizeRefetch(...)`. Retry is bounded (`MaxFinalizeRefetchAttempts=3`) via `RetryFinalizeRefetch`; `finalizeRefetchInProgress` guards re-entry; exhaustion → `completeSnapSync` (batch-2 gate is the backstop). Never finalizes unverified.
- [X] T017 [US3] [P] Coordinator latch tests in `AccountRangeCoordinatorSpec`/`StorageRangeCoordinatorSpec`. DONE: 3 account tests (latched `PivotRefreshed` leaves every task's `(next, rootHash)` and the queue UNCHANGED; post-`EndFinalizing` advance re-tags all to the new root; `BeginFinalizing` re-arms COMPLETED ranges from `rangeStart` with `done=false`, `rootHash=R_final`) + 2 symmetric storage tests (latched `StoragePivotRefreshed` leaves `stateRoot` frozen and does NOT run the non-latched `consecutiveTaskFailures` reset; honored again after `EndStorageFinalizing`). Driven via `TestActorRef` + package-private `finalizeFreezeLatch`. All 5 PASS.
- [X] T018 [US3] The COHERENCE PROOF (`FlatAccountFinalizeCoherenceSpec`). DONE: load `FlatAccountStorage` with a STALE leaf set (`perturbedLeavesAscending` — one account's `storageRoot` from an earlier pivot) → `computeLocalStateRoot == perturbedStateRoot != canonicalStateRoot` (gate FAILS, pre-US3 fail-closed); feed the FINAL-pivot leaves through a real `AccountRangeCoordinator`'s `handleStoreAccountChunk` (the exact worker write path) → the stale leaf is OVERWRITTEN (C1) and `computeLocalStateRoot` flips to `canonicalStateRoot` == R_final, byte-for-byte (gate PASSES). PROVES the re-fetch produces a coherent final root. Wire-level peer-served `GetAccountRange(R_final)` end-to-end + latch-holds-across-live-pivot + controller `FinalizingRefetchComplete→gate→finalize` transition are deferred to the Mordor E2E (T023). Both tests PASS.
