...
| `sync/SyncControllerSpec.scala` | 384 | [Test coverage gap] "start state download only when pivot block is fresh enough" deleted in P10 — wrong params (delta=10 vs threshold=530). No test now covers the pre-start freshness gate (SyncController refusing to issue `FastSync.StartStateSync` when pivot age exceeds threshold). Rewrite: set new peer blockNumber = currentPivot + 531+ so delta ≥ 530 fires the staleness update before state sync begins. Distinct from "update pivot block during state sync if it goes stale" which tests runtime refresh. | EXCEPT | P10 | 2026-06-23 |
