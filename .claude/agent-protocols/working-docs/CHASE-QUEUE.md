...
| ~~`sync/SyncControllerSpec.scala`~~ | ~~243~~ | ~~[DisabledTest P9] "not change best block after receiving faraway block": test injects `PeerRequestHandler.ResponseReceived` directly to Typed FastSync actor via classic ref; `prhResultAdapter` is private (`FastSync.scala:158`) — no external injection path. Needs rewrite to drive FastSync via its Typed Command interface instead.~~ **CLEARED 2026-06-23 (37037a89b, §P9-NOTCHANGE): inject via `WrappedPrhResult` (private[sync]) + `fast.toTyped[FastSync.Command]` — 1/1 pass.** | ~~CLASSIC~~ | EYE | 2026-06-23 |
| ~~`sync/regular/RegularSyncSpec.scala`~~ | ~~550~~ | ~~[DisabledTest P9] "save fetched node": ScalaMock `stub[BranchResolution]` never intercepts `resolveBranch` calls under Scala 3; expectations always unsatisfied. Known Scala 3 reflection issue with ScalaMock stubs. Needs mock → explicit test double.~~ **CLEARED abe9dccc1 (§P9-SAVENODE): replaced all 4 ScalaMock stubs + stateStorage.saveNode.onCall with anonymous class doubles; also override evaluateBranch for PickedBlocks path — 33/34 pass (1 pre-existing failure).** | ~~EXCEPT~~ | EYE | 2026-06-23 |
| ~~`jsonrpc/controllers/JsonRpcControllerSpec.scala`~~ | ~~76, 127~~ | ~~[DisabledTest P9] Two tests: json4s ScalaSig reflection failure under testEssential class-loading order.~~ **CLEARED 2026-06-23 (2803d192f, §P9-JSON4S): Added RpcErrorJsonSerializer to JsonSerializers.formats — DisabledTest removed, 3,600/0 testEssential.** | ~~EXCEPT~~ | EYE | 2026-06-23 |
| ~~`jsonrpc/controllers/JsonRpcControllerEthSpec.scala`~~ | ~~559, 852~~ | ~~[DisabledTest P9] Two tests: same json4s ScalaSig reflection failure as JsonRpcControllerSpec above.~~ **CLEARED 2026-06-23 (2803d192f, §P9-JSON4S)** | ~~EXCEPT~~ | EYE | 2026-06-23 |
| ~~`jsonrpc/service/EthTxServiceSpec.scala`~~ | ~~372~~ | ~~[DisabledTest P9] "returns transaction by hash": `logIndex` expected 1 vs actual 0; topics asserted as `List` but production now returns `Vector`. Assertion drift from production type changes — update expected values and topic container type.~~ **CLEARED 2026-06-23 (21d2a46f0, §P9-TXRECEIPT): `DisabledTest` removed — 24/24 pass; baseLogIndex=1 already correct, stale TODO comment removed.** | ~~EXCEPT~~ | EYE | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~895-897~~ | ~~[Production bug] `handleRegularSyncMsg` catch-all forwards **all** unhandled messages to RegularSync via `regularSync.tell(msg, ctx.toClassic.sender())`. Late-arriving `FastSync.Done` (sent after `syncSwitchDelay`) hits this path and crashes RegularSync with `ClassCastException: FastSync$Done$ cannot be cast to RegularSyncCommand`. Fix: add explicit `case FastSync.Done => Behaviors.same` guard before the catch-all.~~ **CLEARED P10: `case FastSync.Done => Behaviors.same` guard added in P10 commit.** | CLASSIC | EYE | 2026-06-23 |
| ~~`sync/SyncControllerSpec.scala`~~ | ~~384~~ | ~~[Test coverage gap] "start state download only when pivot block is fresh enough" deleted in P10 — wrong params (delta=10 vs threshold=530). No test now covers the pre-start freshness gate.~~ **CLEARED 2026-06-23 (18ceefa5b, P10): Replacement test written — exercises `stalePivotAfterRestart` guard (peers at bestBlock-1 → pivot=399499=current → rejected; peers at bestBlock → pivot=399500>399499 → accepted → stateDownloadStarted).** | ~~EXCEPT~~ | P10 | 2026-06-23 |
| ~~`sync/SyncControllerSpec.scala`~~ | ~~393~~ | ~~[Flaky test] "start state download only when pivot block is fresh enough": intermittent `None.get` at line 437 in full suite. `CombinedRecoveryScanActor` completes (fails → "no gaps") under JVM load BEFORE `GetNodeData` is sent → FastSync concludes → `getSyncState()=None`. Passes 1/1 in isolation. Exposed when P9-NOTCHANGE enabled this as a preceding test.~~ **CLEARED 2026-06-23 (083f08836, §P9-FRESHPIVOT): `.get` → `.map(_.pivotBlock).getOrElse(defaultPivotBlockHeader)` inside `eventually`; 88/88 × 3 consecutive full-suite runs.** | ~~EXCEPT~~ | EYE | 2026-06-23 |

| ~~`sync/BlockchainHostActorSpec.scala`~~ | ~~48~~ | ~~[Pre-existing test failure] "return Receipts for block hashes": `expectMsg(Subscribe(...))` but actor emits `SubscribeCmd(...)`. Classic Pekko Subscribe message vs Typed SubscribeCmd — tag-type mismatch in the assertion. Fails 3/3 consecutive isolation runs. Pre-dates P8 (identical failure before tag changes). Fix: update assertion to expect `SubscribeCmd` (or check if actor should emit classic Subscribe).~~ **CLEARED 07e5d505f: `expectMsg(Subscribe(...))` → `expectMsgType[SubscribeCmd].to shouldBe classifier`; 12/12 pass.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1564~~ | ~~`SNAPSyncController` spawn: `syncController = externalAdapter.toClassic`. Gated on SNAPSyncController LOOM migration (SNAP1 sprint). When SSC becomes a Typed Behavior, update its `syncController: ActorRef` param → `ActorRef[Any]` and drop `.toClassic` here.~~ **CLEARED 0435ac419 (§8k-G2): SNAPSyncController already Typed — IMMEDIATE. `syncController: ActorRef` → `TypedActorRef[Any]`; spawn site updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1725~~ | ~~`ChainDownloader` spawn: `replyTo = externalAdapter.toClassic`. Gated on ChainDownloader LOOM migration.~~ **CLEARED 0435ac419 (§8k-G2): ChainDownloader already Typed — IMMEDIATE. `replyTo: ClassicActorRef` → `TypedActorRef[Any]`; spawn site updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1810~~ | ~~`CombinedRecoveryScanActor` spawn: `externalAdapter.toClassic` positional arg. Gated on CombinedRecoveryScanActor LOOM migration.~~ **CLEARED 0435ac419 (§8k-G2): CRSA already Typed — IMMEDIATE. `syncController: ActorRef` → `TypedActorRef[Any]`; spawn site updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1831, 1852~~ | ~~`BytecodeRecoveryActor` + `StorageRecoveryActor` spawns: `externalAdapter.toClassic` arg. Gated on those actors' LOOM migrations.~~ **CLEARED 0435ac419 (§8k-G2): both already Typed — IMMEDIATE. Params updated; spawn sites updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1909, 1931~~ | ~~Second wave `BytecodeRecoveryActor` + `StorageRecoveryActor` spawns (recovery resume path). Same gate and pattern as above.~~ **CLEARED 0435ac419 (§8k-G2): IMMEDIATE. Both applyPreloaded spawn sites updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| `sync/SyncController.scala` | 1635, 1983 | `RegisterChainWeightCalibrationTarget(externalAdapter.toClassic)` and `RegisterSnapSyncController(externalAdapter.toClassic)`. GATED: these message types do not extend NPMA's sealed `Command` ADT — messages would dead-letter if sent to a Typed NPMA ref without a routing fix. Needs NPMA command routing redesign before `.toClassic` can be dropped. | CLASSIC | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~2137~~ | ~~`PivotHeaderBootstrap` second spawn path: `replyTo = externalAdapter.toClassic`. Gated on PBS LOOM migration or `replyTo` param type update.~~ **CLEARED 0435ac419 (§8k-G2): PBS already Typed — IMMEDIATE. `replyTo: ClassicActorRef` → `TypedActorRef[Any]`; spawn site updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
...
| ~~`sync/SyncController.scala`~~ | ~~548, 582, 746, 1095~~ | ~~`PivotHeaderBootstrap` spawns (four call sites): `replyTo = externalAdapter.toClassic`.~~ **CLEARED 0435ac419 (§8k-G2): IMMEDIATE. All four PBS spawn sites updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |
| ~~`sync/SyncController.scala`~~ | ~~1530~~ | ~~`FastSync` spawn: `syncController = externalAdapter.toClassic`. FastSync already Typed — IMMEDIATE candidate.~~ **CLEARED 0435ac419 (§8k-G2): `FastSync.syncController: ActorRef` → `TypedActorRef[Any]`; spawn site updated.** | ~~CLASSIC~~ | MITHRIL | 2026-06-23 |

| `network/KnownNodesManager.scala:117` + `src/it/.../CommonFakePeer.scala:162` | 117, 162 | [Dead code — compat shim stale] `KnownNodesManager.GetKnownNodes` (case object) was a Classic-only bridge for PeerManagerActor. Comment at line 114 says "Remove once PeerManagerActor is migrated to Typed." PMA IS now Typed (`05e0c003b`). `GetKnownNodes` has 0 production callers. The Classic bridge in `CommonFakePeer.scala:162` (`case KnownNodesManager.GetKnownNodes =>`) is also dead — PeerManager now sends `GetKnownNodesReq(replyTo)`. Remove: (1) `case object GetKnownNodes` from KnownNodesManager.scala + its Scaladoc; (2) `GetKnownNodes` handler + stale comment in CommonFakePeer.scala; (3) confirm no other callers via `grep -rn "GetKnownNodes\b"`. | DEAD | MITHRIL | 2026-06-24 |
| `network/p2p/PeerActor.scala` + `PeerActorSpec.scala:429` | — | [Test failure — migration regression] "should forward PeerClosedConnection with AlreadyConnected to parent": 3s timeout. `PeerActor` no longer sends `PeerClosedConnection` to its Classic parent (removed in `222623960` — `8k-H: remove context.toClassic.parent sends`). Test uses `PropsAdapter(PeerActor.apply(...))` with `parentProbe.ref` as Classic parent, then expects `PeerActor.PeerClosedConnection("127.0.0.1", AlreadyConnected)` to arrive at `parentProbe`. Fix: determine new parent-notification mechanism in Typed PeerActor (typed callback, event bus, or replyTo param), then update PeerActorSpec accordingly. PeerManagerActor is the de-facto parent; AlreadyConnected path may now go through `PeerEventBusActor` — verify. | CLASSIC | MITHRIL | 2026-06-24 |
