...
| ~~F7~~ | ~~Batch F~~ | ~~DEFERRED Part 11 P10 — EYE/MITHRIL FlakyTest root cause audit~~ | ✅ DONE 2026-06-23 — `ab98f1370` — 11 de-tagged (fix), 2 deleted; SyncController FastSync.Done guard added; `18ceefa5b` — coverage gap filled (stalePivotAfterRestart test rewritten, CHASE-QUEUE cleared) |
...
| ~~E5b~~ | ~~Batch F~~ | ~~DEFERRED Part 8 §8a-infra — Any: create `src/test/resources/application-test.conf` (include application.conf + throughput=1) so bare `ScalaTestWithActorTestKit()` ctor works without `ConfigFactory.load()` workaround~~ | ✅ DONE 2026-06-23 — `8b9bef67d` — application-test.conf created; ConfigFactory.load() stripped from 25 specs; bare ctor pitfall resolved |
| ~~E5c~~ | ~~Batch F~~ | ~~DEFERRED Part 8 §8a-infra-b — EYE→LOOM: audit coordinator/heal specs for classic worker teardown leaks; replace `testKit.stop(workerRef)` (silent no-op) with `testKit.system.classicSystem.stop(workerRef)` or confirm coordinator PostStop covers cleanup~~ | ✅ DONE 2026-06-23 — `781c8e985` — audit at `781c8e985`: no leaks found; workers are Typed `context.spawnAnonymous` children (not classic `context.actorOf`), stopped by Pekko hierarchy when coordinator stops; `classicSystem.stop(workerRef)` in ByteCodeCoordinatorSpec is intentional mid-test scenario simulation; 150/150 passed ×2 runs |
| ~~E5d~~ | ~~Batch F~~ | ~~DEFERRED Part 8 §8a-retro batch 4b — MITHRIL: narrow ~209 E165 `TestProbe()` sites in 14 coordinator/heal specs to typed `TestProbe[M]()~~ | ✅ DONE 2026-06-23 — `a193bc794` — 14 coordinator/heal specs; Classic TestProbe() → testKit.createTestProbe[M](); expectMsgType→expectMessageType; fishForMessage→FishingOutcomes; E165 floor 92→65 sites; 141/141 pass; docs `76668d9fd` |
| ~~E5e~~ | ~~Batch F~~ | ~~DEFERRED §8a-infra-c — MITHRIL: replace classic `actorSelection` worker-ref pattern with Typed injection in ByteCodeCoordinatorSpec + AccountRangeCoordinatorSpec~~ | ✅ DONE 2026-06-23 — `5f28e8ae6` — resolveWorkerChild() returns ActorRef[WorkerMessage] via .toTyped[]; all direct actorSelection sends replaced; classicSystem.stop(ref.toClassic) preserves fire-and-forget semantics; 40/40 pass |
...
| ~~E5~~ | ~~Batch G~~ | ~~DEFERRED P12 — MITHRIL tag taxonomy + build target architecture review + gap closure~~ | ✅ DONE 2026-06-24 — `55361ea6f` — 5 new aliases (testConsensus/testRPC/testState/testOlympia/testSync); 15 dead tags removed from Tags.scala; workaround exclusions removed (SyncTest/DisabledTest/FlakyTest) from all 3 tiers + global; testComprehensive now bare; taxonomy at `.local/docs/test-tag-taxonomy.md`. Run `sbt testEssential` to capture post-P12 count (~3,680 expected). |
...
- ~~**Batch F (remaining)**~~: ~~E5d ∥ E5e ∥ §P9-FRESHPIVOT~~ ✅ ALL DONE — E5d `a193bc794`; E5e `5f28e8ae6`; §P9-FRESHPIVOT `083f08836`
