# Fukuii Modernization — Wave 2 Sprint Queue (Archived)

**Branch:** `scala3-cleanup-june`
**Archived:** 2026-06-22 — All Wave 2 and Wave 3 (Part 6) sprints are COMPLETE.
**Active work:** See `working-docs/SPRINT-QUEUE.md` for any ongoing codebase-audit fixes.
...
## Part 6 — Network/P2P Pekko Migration (Wave 3 Completion Sprint) ✅ ALL COMPLETE
...
| 3 | **S1** | Sync recovery atoms ×3 (CombinedRecoveryScanActor, BytecodeRecoveryActor, StorageRecoveryActor) | 697 | 1.5 | LOOM | None | ✅ DONE `ece78c90f` — 11 tests pass; Behavior[Any] + named scanning/downloading; withTimers abandon; watchWith coordinator crash; ctx.self.toClassic as sender() replacement for RequestRecentRoot flow; SyncController 4 spawn sites → context.spawn(...).toClassic (no body surgery). **Sealed Command ADT refinement** (`scala3-cleanup-june`): BytecodeRecoveryActor — 7-case Command ADT (`ScanResult`, `CheckAbandon`, `CoordinatorTerminated`, `ByteCodePeerAvailable`, `ByteCodeDownloadComplete`, `ByteCodeDownloadProgress`, `DroppedBccMsg`); `Behavior[Any]` → `Behavior[Command]` on `apply`/`applyPreloaded`/`testApply`/`scanning`/`downloading`; `ctx.toClassic.actorOf(PropsAdapter(...))` → `ctx.messageAdapter(bccAdapter)` + `ctx.spawn(BCC(snapSyncController = bccAdapter.toClassic))`; `coordinator.toTyped[Nothing] watchWith` → `ctx.watchWith(coordinator, ...)`. StorageRecoveryActor — 9-case Command ADT (`ScanResult`, `CheckAbandon`, `CoordinatorTerminated`, `StoragePeerAvailable`, `StorageRangeDone`, `StorageSlotProgress`, `PivotUnservable`, `DroppedSrcMsg`, `RecentRoot`); `RequestRecentRoot` case object → `case class RequestRecentRoot(replyTo: ActorRef)` (sent, not received); `Behavior[Any]` → `Behavior[Command]`; `srcAdapter.toClassic` as snapSyncController; `syncController.tell(RequestRecentRoot, ctx.self.toClassic)` → `syncController.tell(RequestRecentRoot(ctx.self.toClassic), noSender)`. SyncController — 3 targeted changes: peer-available sends use local Command types; `case RequestRecentRoot =>` + `sender()` → `case RequestRecentRoot(replyTo) =>`. Both specs updated; `SNAPSyncController` imports removed. 248/248 pass. Commit `ee2e94156` (5 files, 134 ins, 125 del). |
| 4 | **S2** | Fast sync leaf ×2 (StateStorageActor, FastSyncBranchResolverActor) | 398 | 0.5 | LOOM | None | ✅ DONE — StateStorageActor `9f20ec92b`, FastSyncBranchResolverActor `22bbdb926` (PLN session — `Behavior[Any]` proof-of-concept). **Sealed Command ADT refinement** (`scala3-cleanup-june`): 6-case Command ADT (`StartBranchResolver`, `ScanPeers`, `HandlerTerminated`, `HandshakedPeersMsg`, `PeerDisconnectedMsg`, `PeerRequestResult`); 5 behaviors narrowed to `Behavior[Command]`; `handshakedPeersAdapter` param added to Resolver (created in `apply`, threaded down — same pattern as BlockBroadcasterActor). 25/25 pass. Commit `9b7ed2004`. testEssential pending. |
...
| 6 | **S6** | ChainDownloader | 897 | 1.0 | LOOM | PLN | ✅ DONE `581376757` — Behavior[Any], both SyncController + SNAPSyncController spawn sites updated, PeerListHelper wired, 16 tests pass. ⚠️ Metric note: E165 = unmatchable-Any (rises with Behavior[Any]). **Sealed Command ADT narrowing** (`scala3-cleanup-june`): `Behavior[Command]` with sealed ADT; 3 bridge wrappers (`PeerResult`, `HandshakedPeersMsg`, `PeerGone`) covering all Classic bridges; `context.self.toClassic` poll sites → `handshakedPeersAdapter.toClassic`; `SNAPSyncController.chainDownloader: Option[ActorRef[ChainDownloader.Command]]` — sends now type-safe. compile-all clean. 23/23 pass. Commit `d1c744c07`. |
| 7 | **S5** | Regular sync ×3 (BlockBroadcasterActor, BlockImporter, RegularSync) | 1,120 | 2.0 | LOOM | W1 + PLN | ✅ DONE `5d29511d4` — BlockBroadcasterActor: Behavior[BroadcasterMsg]; BlockImporter+RegularSync: Behavior[Any] (mixed Classic+Typed sources); PeerListHelper replaces PeerListSupportNg; 19 Classic actors remain. 69 pre-existing jsonrpc failures fixed `92584a07b`. Baseline: 3,519 / 0 failures. **Sealed Command ADT narrowing** (`scala3-cleanup-june`): BlockImporter → `Behavior[Command]` (sealed 9-case ADT; `FetcherResponse`/`BranchResolverMsg` wrappers; `branchResolverAdapter` identity → `BranchResolverMsg(_)`; new `fetcherReplyTo`; file-level `import BlockImporter.Command` for constructor scope); RegularSync → `Behavior[Command]` (`type Command = SyncProtocol.RegularSyncCommand`; timer ticks + ProgressProtocol extend it; `ctx.self.narrow[ProgressProtocol]` for BlockFetcher supervisor; Classic refs to SyncController preserved); SyncProtocol.scala — `trait RegularSyncCommand` added, 4 cases extended; BlockFetcher.scala — supervisor field `ClassicActorRef` → `ActorRef[RegularSync.ProgressProtocol]`; RegularSyncItSpecUtils.scala — blockImporter typed, `regularSync.toTyped[ProgressProtocol]` for BlockFetcher factory. 48/48 pass. Commit `5e3908f7f` (5 files, 80 ins, 53 del). |
...
| 10 | **S4** ✅ | SyncStateSchedulerActor + PivotBlockSelector | 987 | 2.0 | HERALD+LOOM | NET + PLN | ✅ DONE `df703539e` — shell+core (Behavior[Any]); `activeHandlers: Map[PeerId, ClassicRef]` replaces `sender()`; `fiberLog` for CE IO-fiber logging (critical bug: `ctx.log` on compute thread → UnsupportedOperationException, silently swallowed by pipeToSelf, stalling sync); 5/5 StateSyncSpec. 3,593 / 0 (exit 0, 705s). **S4a — SyncStateSchedulerActor ADT narrowing** ✅ `8a46f7735` (2026-06-21): 13-case sealed Command ADT (8 existing + 5 new wrappers: `WrappedPRHResult`, `WrappedHandshakedPeers`, `WrappedPeerDisconnected`, `WrappedRequestData`, `WrappedRequestFailed`); 10 `ctx.self.toClassic` self-send sites fixed (IO fiber ×2, startSyncing, PRH result forwarding ×4 incl. RequestTerminated, ProcessingResult, DownloaderError ×2); 2 NPMA `GetHandshakedPeers` sites → `handshakedPeersAdapter.toClassic`; 2 identity adapter bugs fixed (`PeerDisconnected` + `PeerRequestHandler.Result`); callers updated: `FastSync.scala:333` + `StateSyncSpec.scala:254` (`TypedActorRef[Any]` → `TypedActorRef[SyncStateSchedulerActor.Command]`). compile-all [success]. 3 files. **S4b — PivotBlockSelector ADT narrowing** `e82f41cac` (2026-06-21, `PivotBlockSelector.scala` only): 6-case sealed Command ADT (`SelectPivotBlock`, `ElectionPivotBlockTimeout`, `ScanPeers` existing; `WrappedMessageFromPeer`, `WrappedHandshakedPeers`, `WrappedPeerDisconnected` new); 3 adapters in `apply()` (`peerDisconnectedAdapter` identity fixed, `blockHeadersAdapter`, `handshakedPeersAdapter`); all 6 subscription sites updated to `blockHeadersAdapter.toClassic` (SubscribeCmd L315, UnsubscribeAllCmd ×4 L153/239/269/309, UnsubscribeCmd L218-220); both NPMA reply sites → `handshakedPeersAdapter.toClassic`; SendMessage site → `blockHeadersAdapter.toClassic`; `pivotBlockRetryCount`/`totalSelectionAttempts`/`pivotRetryState` eliminated from Impl — threaded as immutable `PivotState` through all behavior factories; timer keys `ElectionTimeoutKey`/`RetryKey`/`ScanKey` → typed case objects; `Behavior[Any]` → `Behavior[Command]` throughout. compile-all 0 errors (23 pre-existing warnings in FastSync + PivotBlockSelectorSpec, none in PivotBlockSelector); scalafmtAll 1 reformatted. **PivotBlockSelectorSpec fix** ✅ `403657de3` — Problem 1: `Subscribe(classifier)` → `SubscribeCmd(classifier, _)` (~29 items via `expectSubscribeCmds` helper); `Unsubscribe(Some(c))` → `UnsubscribeCmd(c, _)` (~26 items via 3 helpers); `Unsubscribe(None/empty)` → `UnsubscribeAllCmd` (8 items); `ignoreMsg` updated (2+1 sites); 4 helper methods added to TestSetup. Problem 2: `pivotBlockSelector ! MessageFromPeer(...)` → `WrappedMessageFromPeer(...)` (26); `pivotBlockSelector ! handshakedPeers` → `WrappedHandshakedPeers(handshakedPeers)` (1 — undocumented in pre-flight, root cause of all test timeouts). compile-all 0 errors (4 pre-existing Matchable from TestKit `ignoreMsg`). 34/34 pass (PivotBlockSelectorSpec 12 + SyncStateDownloaderStateSpec 11 + SyncStateSchedulerSpec 11). **S4 fully closed.** |
| 11 | **NET2** | NetworkPeerManagerActor | 1,442 | 2.0 | HERALD+LOOM | NET, S3 ADT defined, HERALD-3 | ✅ DONE `5b8762900` — shell+core split; Impl class (8 var + 3 mutable coll); withTimers ×3; 2 ask-path replies at shell; snapSyncControllerOpt stays Option[ActorRef] (Classic). 184/184 targeted, 3,601/0 testEssential. **Command ADT narrowing** ✅ `be305095f` — Option A (Classic shell + narrow core): `NetworkPeerManagerShell` (new Classic Actor, same file) absorbs all 9 legacy message types (`GetHandshakedPeers`, `PeerInfoRequest`, `SendMessage`, `RegisterSnapSyncController`, `RegisterChainWeightCalibrationTarget`, `CalibrateChainWeightNow`, `UpdateClHead`, `ConnectToPeer`, raw `PeerEvent`); captures `sender()` for 2 ask-path replies; wraps all into Command and forwards to Typed core child. Core: `Behavior[Any]` → `Behavior[Command]` throughout (`apply()`, `handleMessages`, `handlePeerHandshakeSuccessful`, `TypedActorContext` type param); 28-line Classic absorption block removed. NodeBuilder: spawns `NetworkPeerManagerShell.props(...)` (return type stays untyped `ActorRef`, no callers changed). 5 test files updated to spawn shell. compile-all clean, scalafmtAll clean, 131/131 targeted. |
...
| 13 | **SNAP2** | FastSync | 1,415 | 2.5 | LOOM | S2, S4, S7 | ✅ DONE — Phase 1 `df59c326c` (sealed Command ADT), Phase 2 `501255dfd` (Classic shell + `Behavior[Any]` core, shell+core pattern as S4/S7; PeerListSupportNg→PeerListHelper; 7 context.become→behaviors; raw scheduler→withTimers; Classic PRH children via ctx.toClassic.actorOf, watchWith→RequestTerminated; 11 log.warning→log.warn; syncController explicit). Phase 3 no-op: shell preserves Classic `FastSync.props`, SyncController `context.actorOf` unchanged. Phase 4: FastSync targeted 34/35 (1 fail = pre-existing "does not crash" 60s timeout, verified identical on baseline `4d6fcdf6a` — NOT a regression); testEssential **3,593 / 0** (3491 main + 26 rlp + 11 bytes + 65 crypto), exit 0. P11 clean (core has no Future/IO/pipeToSelf; single `ctx.log` captured as SLF4J `log` member at init). **Command ADT narrowing (SNAP2-a)** ✅ `e41f50b7d` — ADT rebuilt from scratch (prior ADT from `df59c326c` was removed by later shell deletions): 8 internal + 7 wrapper cases (`WrappedSyncProtocol`, `WrappedHandshakedPeers`, `WrappedPeerDisconnected`, `WrappedPivotResult`, `PivotSelectionFailed`, `WrappedPrhResult`, `WrappedSchedulerResponse`, `WrappedBranchResolverResponse`); `WrappedStateSyncStats` added as dedicated case (`StateSyncStats` doesn't extend `SyncStateSchedulerActorResponse`); `messageAdapter[BranchResolverResponse](identity)` → `WrappedBranchResolverResponse(_)` (CHASE-QUEUE silent-drop fix); all 20 `Behavior[Any]` sites flipped (return types, generics, `var nextBehavior` locals); 10 type-mismatch compile errors resolved by routing foreign types through Wrapped* constructors; 3 `PivotBlockSelector` spawns + 1 SSC spawn rewired from `ctx.self.toClassic` to adapters; `WrappedSyncProtocol` relaxed `private[fast]` → `private[sync]` for SyncController cross-file wrapping (2 send sites); `handlePeerList`/`handleStatus` narrowed from `Any` to `Command`; 3 test call sites fixed (`FastSyncSpec` raw-send → wrapped); `FastSyncBranchResolverActorSpec` load-contention timeouts pre-existing, not a regression. 3 files (FastSync.scala, SyncController.scala, FastSyncSpec.scala). compile-all 0 errors, scalafmtAll clean. 3 residual E165 at L502/534/551 — intrinsic to `PRH ResponseReceived[?]` protocol. |
...
These tasks ran **independently** during `testEssential` (~24 min) and `testStandard` (~30 min)
wait times. No gate dependency on the primary sprint groups above.
