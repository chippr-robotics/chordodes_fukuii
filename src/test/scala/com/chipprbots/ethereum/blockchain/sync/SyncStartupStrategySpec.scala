...
class SyncStartupStrategySpec extends AnyFunSuite with ParallelTestExecution with TestSyncConfig {
  ...
  test("downgrades to Fast when only 1 peer is snap-capable (below threshold of 3)", UnitTest, SyncTest) {
    assert(selectSyncMode(5, 1, 0L, snapCfg) == SyncMode.Fast)
  }

  test("returns Snap when exactly 3 snap-capable peers are available", UnitTest, SyncTest) {
    assert(selectSyncMode(5, 3, 0L, snapCfg) == SyncMode.Snap)
  }

  test("returns Snap when snap-capable peers are the majority", UnitTest, SyncTest) {
    assert(selectSyncMode(10, 8, 0L, snapCfg) == SyncMode.Snap)
  }

  test("returns Fast for fast-only config regardless of peer snap capability", UnitTest, SyncTest) {
