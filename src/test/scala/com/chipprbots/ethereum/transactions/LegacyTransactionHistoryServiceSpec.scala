...
class LegacyTransactionHistoryServiceSpec
  ...
  "returns account recent transactions in newest -> oldest order" in testCaseM { (fixture: Fixture) =>
    ...
    val tx1 =
      SignedTransaction.sign(LegacyTransaction(0, 123, GasAmount(456), Some(address), 1, ByteString()), keyPair, None)
    val tx2 =
      SignedTransaction.sign(LegacyTransaction(0, 123, GasAmount(456), Some(address), 2, ByteString()), keyPair, None)
    val tx3 =
      SignedTransaction.sign(LegacyTransaction(0, 123, GasAmount(456), Some(address), 3, ByteString()), keyPair, None)
  ...
  "does not return account recent transactions from older blocks and return pending txs" in testCaseM {
    (fixture: Fixture) =>
      ...
      val tx = LegacyTransaction(0, 123, GasAmount(456), None, 99, ByteString())
