...
/** STOP-AND-ALERT death-watch for a critical actor that must NOT be restarted (restart corrupts state or silently drops
  ...
  * hot-path cost). A standalone alerter is spawned alongside it and `watchWith`-es the child. When the child terminates,
  * the alerter logs a `CRITICAL` error (the keyword ops alerting greps for) and stops. There is deliberately no
  * `SupervisorStrategy` restart — the node is left in a loudly-flagged degraded state requiring a controlled restart,
  * rather than a silent recovery that hides the failure.
  */
