...
trait WorldStateProxy[WS <: WorldStateProxy[WS, S], S <: Storage[S]] { self: WS =>
  ...
  /** EIP-7610 (Prague): a create at `address` must revert if the account has non-empty code, non-zero nonce, OR
    * non-empty storage. Extends EIP-684 which checked only code+nonce.
    */
