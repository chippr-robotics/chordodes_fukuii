        ...
        case ScanResult(missing) =>
          if missing.isEmpty then {
            ...
            missing.grouped(batchSize).foreach { batch =>
              ...
              coordinator.tell(actors.Messages.AddStorageTasks(tasks), org.apache.pekko.actor.ActorRef.noSender)
    ...
    Behaviors.withTimers { timers =>
      ...
      Behaviors.receiveMessage {
        case actors.Messages.StoragePeerAvailable(peer) =>
          coordinator.tell(actors.Messages.StoragePeerAvailable(peer), org.apache.pekko.actor.ActorRef.noSender)
          Behaviors.same
        ...
        case RecentRoot(blockNumber, rootOpt) =>
          ...
          rootOpt match {
            case Some(root) if root != currentRoot =>
              ...
              coordinator.tell(actors.Messages.StoragePivotRefreshed(root), org.apache.pekko.actor.ActorRef.noSender)
            case Some(_) =>
              ctx.log.info(
